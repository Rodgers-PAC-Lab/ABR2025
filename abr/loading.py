...
def get_ABR_data_paths(json_filepath):
    # json_filepath points to the json file that stores all the filepaths for
    # various computer which might run this script.
    ...
    for computer in data['computers']:
        computer_l.append(computer)
    computers_df = pandas.DataFrame(computer_l).set_index('computer_ID')
    current_rawdata_dir = os.path.expanduser(computers_df.loc[current_computer]['rawdata_directory'])
    current_pickle_dir = os.path.expanduser(computers_df.loc[current_computer]['pickle_directory'])
    return current_rawdata_dir,current_pickle_dir
